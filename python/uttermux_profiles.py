...
def profiles() -> list[dict]:
    ...
    for manifest in sorted(root.glob("*/*/profile.json")):
        try:
            ...
            if document.get("schemaVersion") != SCHEMA_VERSION: continue
            reference = manifest.parent / document.get("referenceFile", "")
            artifact = manifest.parent / document.get("artifactFile", "")
            document["directory"] = str(manifest.parent)
            document["referencePath"] = str(reference) if reference.is_file() else ""
            document["artifactPath"] = str(artifact) if artifact.is_file() else ""
            document["available"] = reference.is_file() or artifact.is_file()
...
def export_profile(profile_id: str, destination: Path) -> Path:
    ...
    with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as archive:
        for name in ("profile.json", item.get("referenceFile", ""), item.get("artifactFile", "")):
...
def import_profile(bundle: Path) -> dict:
    with zipfile.ZipFile(bundle) as archive:
        ...
        if document.get("schemaVersion") != SCHEMA_VERSION or document.get("engine") not in set(ENGINES) | {"qwen"}:
            raise ValueError("unsupported voice bundle schema or engine")
        allowed = {"profile.json", document.get("referenceFile", ""), document.get("artifactFile", "")}
